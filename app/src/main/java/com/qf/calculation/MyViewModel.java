...
public class MyViewModel extends AndroidViewModel {
    ...
    private static String KEY_SAVE_SHP_DATA_NAME = "key_save_shp_data_name";
    ...
    public MyViewModel(@NonNull Application application, SavedStateHandle handle) {
        ...
        if(!handle.contains(KEY_HIGH_SCORE)){
            SharedPreferences shp = getApplication().getSharedPreferences(KEY_SAVE_SHP_DATA_NAME, Context.MODE_PRIVATE);
    ...
    public MutableLiveData<Integer> getCurrrentScore(){
    ...
    public void answerCorrent(){
        getCurrrentScore().setValue(getCurrrentScore().getValue() + 1);
        if(getCurrrentScore().getValue() > getHighScore().getValue()){
            getHighScore().setValue(getCurrrentScore().getValue());
            win_flag = true;
        }
